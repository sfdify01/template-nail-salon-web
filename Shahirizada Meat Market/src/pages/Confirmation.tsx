...
import { CheckCircle2, Clock, MapPin, ShoppingBag, Truck, ArrowRight, Phone, Star, Gift } from 'lucide-react';
import { Card } from '../components/ui/card';
import { Button } from '../components/ui/button';
import { Separator } from '../components/ui/separator';
import { formatCurrency } from '../lib/pricing';
import { getPickupCode } from '../lib/utils/id';
import { earnStars } from '../lib/loyalty/client';
import { EarnResponse } from '../lib/loyalty/types';
...
export const Confirmation = ({ orderId, onNavigate, config }: ConfirmationProps) => {
  ...
  const [earnResult, setEarnResult] = useState<EarnResponse | null>(null);
      ...
      if (foundOrder) {
        ...
        try {
          const result = earnStars(
            foundOrder.id,
            Math.round(foundOrder.totals.grand_total * 100),
          ...
          >
            <Card className="p-6 mb-6 bg-gradient-to-br from-amber-50 to-orange-50 border-amber-200">
              ...
              <div className="bg-white rounded-lg p-4 space-y-2">
                ...
                {earnResult.referralCredited && (
                  <>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-700">Referral bonus (you)</span>
                      <span className="font-semibold text-green-600">
                        +{earnResult.referralCredited.friendBonus} ⭐
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-700">Referral bonus (friend)</span>
                      <span className="font-semibold text-green-600">
                        +{earnResult.referralCredited.inviterBonus} ⭐
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2 p-2 bg-green-50 rounded-lg">
                      ...
                      <p className="text-xs text-green-700">
                        Referral bonus applied! Your friend earned +{earnResult.referralCredited.inviterBonus} ⭐ too.
                      </p>
                    </div>
                  </>
                )}
