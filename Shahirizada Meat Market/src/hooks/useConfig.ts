...
const configData: Config = {
  ...
  social: {
    instagram: "shahirizadameatmarket",
    facebook: ""
  },
  seo: {
      ...
      items: [
        {
          id: "avocado-toast",
          name: "Avocado Toast",
          description: "Sourdough, smashed avocado, cherry tomatoes, microgreens, poached egg",
          price: 14.00,
          image: "breakfast avocado",
          dietary: ["vegetarian"],
          popular: true
        },
        {
          ...
          description: "Caramelized bananas, maple syrup, whipped cream",
          price: 13.00,
          image: "french toast",
          dietary: ["vegetarian"]
        },
      ...
      items: [
        {
          id: "truffle-burger",
          name: "Truffle Burger",
          description: "Angus beef, truffle aioli, caramelized onions, arugula, brioche bun",
          price: 18.00,
        ...
        },
        {
          id: "caesar-salad",
          name: "Caesar Salad",
          description: "Romaine, parmesan, house-made croutons, classic dressing",
          price: 14.00,
          image: "caesar salad",
          dietary: ["vegetarian"],
          addOns: ["Grilled Chicken +$6", "Grilled Salmon +$8"]
        },
        {
          id: "salmon-bowl",
          name: "Grilled Salmon Bowl",
          description: "Atlantic salmon, quinoa, roasted vegetables, tahini dressing",
          price: 22.00,
          image: "salmon bowl",
          dietary: ["gluten-free"],
          popular: true
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          id: "cheesecake",
          name: "NY Cheesecake",
          description: "Creamy cheesecake, berry compote, graham cracker crust",
          price: 8.00,
          image: "cheesecake",
          dietary: ["vegetarian"]
        }
  ...
  useEffect(() => {
    ...
    const fetchMenu = async () => {
      try {
        ...
        const response = await fetch(
          `https://${projectId}.supabase.co/functions/v1/make-server-a05c3297/menu`,
          {
            headers: {
        ...
        if (response.ok) {
          ...
          // If server has menu data, use it; otherwise use fallback
          if (data && data.categories && data.categories.length > 0) {
            console.log('✅ Menu loaded from server:', data.categories.length, 'categories');
            setMenu(data);
          } else {
            console.log('ℹ️ No menu data on server yet. Using local fallback. Visit /migrate-menu to upload your menu.');
            setMenu(menuData);
          }
        } else {
          // Fallback to hardcoded data if server fails
          console.log('ℹ️ Server menu fetch failed. Using local fallback. Visit /migrate-menu to upload your menu.');
          setMenu(menuData);
        }
      } catch (error) {
        // Fallback to hardcoded data on error
        console.log('ℹ️ Menu fetch error (using local fallback):', error.message || error);
        setMenu(menuData);
      }
    };
    ...
    const fetchBlogPosts = async () => {
      try {
        const { projectId, publicAnonKey } = await import('../utils/supabase/info');
        const response = await fetch(
          `https://${projectId}.supabase.co/functions/v1/make-server-a05c3297/blog/posts`,
          {
            headers: {
              'Authorization': `Bearer ${publicAnonKey}`,
            },
